...
static bseSurfaceBuilder_t &SurfaceForMaterial( idList<bseSurfaceBuilder_t> &surfaces, const idStr &materialName ) {
	for ( int i = 0; i < surfaces.Num(); i++ ) {
		if ( !surfaces[i].materialName.Icmp( materialName ) ) return surfaces[i];
	}
...
void BSE_BuildRenderModel( idRenderModelStatic *snapshot, const char *snapshotName,
	...
	for ( int i = 0; i < particles.Num(); i++ ) {
		...
		bseSurfaceBuilder_t &surface = SurfaceForMaterial( builders,
			particle.materialName.IsEmpty() ? idStr( "_default" ) : particle.materialName );
